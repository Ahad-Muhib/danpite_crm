...
class QuotationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'subject': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. Quotation for CRM & HRM Software Solution'}),
            'intro_letter': forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'Write introductory greeting message...'}),
            'status': forms.Select(attrs={'class': 'form-select'}),
            'currency': forms.Select(attrs={'class': 'form-select'}),
            'total_amount': forms.NumberInput(attrs={'class': 'form-control', 'min': '0', 'step': '0.01'}),
            'payment_terms': forms.Textarea(attrs={'class': 'form-control', 'rows': 5, 'placeholder': 'e.g. 40% Advance, 30% Before Testing, 30% Before Delivery...'}),
            'delivery_terms': forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'e.g. Standard delivery within 15 working days...'}),
            'terms_conditions': forms.Textarea(attrs={'class': 'form-control', 'rows': 5, 'placeholder': 'e.g. Validity 15 days, Additional customization charged separately...'}),
            'warranty': forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'e.g. 3 Months free bug fix warranty...'}),
            'not_included': forms.Textarea(attrs={'class': 'form-control', 'rows': 4, 'placeholder': 'e.g. Third-party SMS gateway fees, Server domain costs...'}),
            'why_danpite': forms.Textarea(attrs={'class': 'form-control', 'rows': 4, 'placeholder': 'e.g. Custom development, Modern UI/UX, Dedicated support...'}),
        }
    ...
    def clean(self):
        ...
        if not client:
            self.add_error('client_search', 'A registered client must be selected from the system. Quotations cannot be created without selecting a registered client.')
            return cleaned
